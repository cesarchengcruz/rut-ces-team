...
class Cube {
  constructor() {
    ...
    this.directionalLight = new THREE.DirectionalLight(0xe8c37b, 0.5);
    ...
    function addBox(label) {
      switch (label) {
        ...
        case "room":
          ...
          break;
        case "connector":
          game.counter.connector += 1;
          break;
        case "bamboo":
          game.counter.bamboo += 1;
          break;
        case "stairs":
          game.counter.stairs += 1;
          break;
      }
      // console.log(counter)
      calcNeu(game.counter);
    }

    function updateCount(e) {
      game.data = calcNeu(game.counter);
      game.answer = document.getElementById("calculated-area");

      game.area = calcNeu(game.counter);
      // Restrict the area to 2 decimal points.
      game.rounded_area = Math.round(game.area * 100) / 100;
      game.answer.innerHTML = `<p><strong>${game.rounded_area}</strong></p><p>neutrality ratio</p>`;
    }
    ...
    const game = this;
    this.model01.load("static/models/module-room.glb", function (gltf) {
    ...
    this.model02.load("static/models/module-roof.glb", function (gltf) {
      let model = gltf.scene;
      gltf.scene.traverse((o) => {
        if (o.isMesh) {
          o.castShadow = true;
    ...
    this.model03.load("static/models/module-stairs.glb", function (gltf) {
      let model = gltf.scene;
      gltf.scene.traverse((o) => {
        if (o.isMesh) {
          o.castShadow = true;
    ...
    this.model04.load("static/models/module-connector.glb", function (gltf) {
      let model = gltf.scene;
      gltf.scene.traverse((o) => {
        if (o.isMesh) {
          o.castShadow = true;
    ...
    document.addEventListener("pointerdown", (event) => {
      ...
      console.log("CCCCCCCCCCCC", intersects);
      if (intersects.length > 0) {
        const intersect = intersects[0];
